...
function MesTransporteurs(props) {
	...
	const fetchTransporteurs = async (page) => {
		...
		const res = await getValidTransporters(page, perPage)
	...
	const searchForTransporteurs = async (q) => {
		if (q.length === 0) {
			return
		}
		...
		const res = await fullTextSearchTransporteurs(q)
		setData(res.hits)
			...
			>
				...
				>
					<input
						...
						onKeyDown={(e) => {
							if (e.key === 'Enter') {
								searchForTransporteurs(searchQuery)
							}
						}}
					...
					<button
						disabled={loading}
						onClick={() => searchForTransporteurs(searchQuery)}
						style={{
							padding: '1rem 2rem',
							background: '#50F5A9',
							border: 'none',
