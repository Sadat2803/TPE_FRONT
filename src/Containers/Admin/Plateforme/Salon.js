...
function Salon(props) {
	...
	const fetchCommandes = async (page) => {
		...
		const res = await getCommandesForSalon(page, perPage)
	...
	const handlePerRowsChange = async (newPerPage, page) => {
		...
		const res = await getCommandesForSalon(page, newPerPage)
				...
				>
					...
					<button
						onClick={async () => {
							...
							await validateAttributionCommandes(
								selected.map((c) => c._id)
							)
