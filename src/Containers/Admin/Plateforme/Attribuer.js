...
function Attribuer(props) {
	...
	const fetchCommandes = async (page) => {
		...
		const res = await getCommandesWithTransporter(page, perPage)
	...
	const handlePerRowsChange = async (newPerPage, page) => {
		...
		const res = await getCommandesWithTransporter(page, newPerPage)
	...
	const fetchTransporters = async () => {
		// TODO change this
		const res = await getCommandesWithTransporter(1, 50)
	...
	const columns = [
		...
		},
		{
			...
			selector: (row) => {
				...
				if (row.type_commande === 'course-a-course') {
					let $t = [...row.courses].pop()
					if ($t?.time) {
						return $t.time
					} else {
						return $t.heure_debut
					}
				}
			},
			sortable: false,
